...
const productType = (productType) => {
    ...
    cartShoppings.forEach( (cartShoppingsProduct) => {
        if (cartShoppingsProduct.type === productType){
...
const productPriceFilter = (condition, value) => {
    let productsFilteredArray = [];
    if (condition == "mayor"){
        cartShoppings.forEach( (cartShoppingsProduct) => {
            if (cartShoppingsProduct.price > value){
                productsFilteredArray.push(cartShoppingsProduct)
            }
        });
    ...
    } else if (condition == "menor o igual"){
      cartShoppings.forEach( cartShoppingsProduct => {
        if (cartShoppingsProduct.price <= value){
          productsFilteredArray.push(cartShoppingsProduct)
          }
    ...
    }else if (condition == "mayor o igual"){
      cartShoppings.forEach( cartShoppingsProduct => {
        if (cartShoppingsProduct.price >= value){
          productsFilteredArray.push(cartShoppingsProduct)
        }
    ...
    return console.log(`Los productos con un precio ${condition} a ${value} son:\n`, productsFilteredArray)

  }

productPriceFilter("mayor o igual", 50)
...
const allProductsValue = () => {
  let totalValue = 0
  cartShoppings.forEach( cartShoppingsProduct => {
    totalValue = totalValue + cartShoppingsProduct.price
  })
  return totalValue
}

console.log("El valor total de todos los productos es: ", allProductsValue())
...
const allProductsValuePerType = type => {
  let productTypeValue = 0
  cartShoppings.forEach( cartShoppingsProduct => {
    if(cartShoppingsProduct.type == type) {
      productTypeValue += cartShoppingsProduct.price
    }
  })
  return console.log(`El valor total de inventario de los productos tipo ${type} es`, productTypeValue.toFixed(2))

}

allProductsValuePerType("drink")
